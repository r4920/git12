...
async function seedUser () {
  try {
    let user = await User.findOne({
      'username':'Judge.Thompson63',
      'isDeleted':false,
      'isActive':true
    });
    if (!user || !user.isPasswordMatch('KZB8KublXrzGdXC') ) {
      let user = new User({
        'password':'KZB8KublXrzGdXC',
        'username':'Judge.Thompson63',
    ...
    let admin = await User.findOne({
      'username':'Brant_Renner57',
      'isDeleted':false,
      'isActive':true
    });
    if (!admin || !admin.isPasswordMatch('0xXtZ0PuXfnA7fz') ) {
      let admin = new User({
        'password':'0xXtZ0PuXfnA7fz',
        'username':'Brant_Renner57',
...
async function seedUserRole (){
  try {
    ...
    if (user && user.isPasswordMatch('KZB8KublXrzGdXC') && userRole){
    ...
    if (admin && admin.isPasswordMatch('0xXtZ0PuXfnA7fz') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id
      });
